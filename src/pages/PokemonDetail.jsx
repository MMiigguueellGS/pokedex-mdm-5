    ...
    <main className=" grid gap-8 font  max-w-[1190px] mx-auto p-2 py-28 dark:text-white px-10">
      <article className="">
        ...
        >
          <Link
            to="/pokedex/"
            className="absolute -top-[100%] z-10 text-white bg-zinc-700 font-bold text-sm rounded-full  inline-block py-1 px-5 shadow-lg shadow-slate-800/60 dark:bg-white dark:text-black transition-all ease-in-out "
          >
            Go Back
          </Link>
          <img
            ...
            <div className=" grid gap-4 mt-4">
              <span
                className={`text-3xl font-bold ${
                  nameStylePokemon[pokemonData?.types[0]]
                } `}
              >
                #{pokemonData?.id}
              </span>
              <h2
                className={`capitalize text-5xl font-bold  ${
                  nameStylePokemon[pokemonData?.types[0]]
                }`}
              >
                {pokemonData?.name}
              ...
              <div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {pokemonData?.abilities.map((abilitie) => (
                    <span
                      key={abilitie}
                      className="px-5 py-1 text-md font-semibold border-solid border-2 border-gray-200 rounded shadow-lg shadow-slate-700/20"
                    >
                      {abilitie}
                    </span>
                  ))}
